...
public class DatabaseManager {
    ...
    // ===============================================
    // MÉTODOS AUXILIARES Y DE OPTIMIZACIÓN
    // ===============================================
    
    /**
     * Verifica si la base de datos está disponible y funcional
    ...
    public static boolean verificarConexion() {
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            logger.error("Error verificando conexión a la base de datos: {}", e.getMessage());
            return false;
        }
    }
     ...
     */
    public static String obtenerEstadisticasBaseDatos() {
        try {
            int totalProductos = leerProductos().size();
            int totalVentas = leerVentas().size();
            double totalVentasHoy = obtenerTotalVentasHoy();
            int conteoVentasHoy = obtenerConteoVentasHoy();
            
            return String.format(
                "Estadísticas DB: Productos=%d, Ventas=%d, VentasHoy=%d, TotalHoy=%.2f",
                totalProductos, totalVentas, conteoVentasHoy, totalVentasHoy
            );
        } catch (Exception e) {
            logger.error("Error obteniendo estadísticas de base de datos: {}", e.getMessage());
    ...
    public static Producto buscarProductoPorNombre(String nombre) {
        ...
        try {
            ...
            logger.error("Error buscando producto por nombre: {}", e.getMessage());
            return null;
        }
    }
    
    /**
     * Obtiene productos con stock bajo (menor a un umbral específico)
     * @param umbralStock Umbral mínimo de stock
     * @return Lista de productos con stock bajo
     */
    public static List<Producto> obtenerProductosConStockBajo(int umbralStock) {
        try {
            List<Producto> productos = leerProductos();
            return productos.stream()
                           .filter(p -> p.getStock() < umbralStock)
     ...
     */
    public static boolean actualizarStockProducto(String nombreProducto, int nuevoStock) {
        if (nombreProducto == null || nombreProducto.trim().isEmpty() || nuevoStock < 0) {
            return false;
        }
        ...
        try {
            ...
            logger.error("Error actualizando stock del producto {}: {}", nombreProducto, e.getMessage());
            return false;
        }
    }
     ...
     * @return true si todos los datos están consistentes, false en caso contrario
     */
    public static boolean validarIntegridadDatos() {
        try {
            // Verificar que no hay productos con nombres nulos o vacíos
            List<Producto> productos = leerProductos();
            boolean productosValidos = productos.stream()
                                                .allMatch(p -> p.getNombre() != null && 
                                                              !p.getNombre().trim().isEmpty() &&
                                                              p.getPrecio() >= 0 &&
                                                              p.getStock() >= 0);
            
            // Verificar que no hay ventas con datos inconsistentes
            List<Venta> ventas = leerVentas();
            boolean ventasValidas = ventas.stream()
                                          .allMatch(v -> v.getId() != null && 
                                                        !v.getId().trim().isEmpty() &&
                                                        v.getTotal() >= 0);
            
            return productosValidos && ventasValidas;
        } catch (Exception e) {
            logger.error("Error validando integridad de datos: {}", e.getMessage());
            return false;
        }
    }
    
    /**
     * Obtiene el conteo total de registros en la base de datos
     * @return Array con [productos, ventas, operaciones_caja]
     */
    public static int[] obtenerConteoTotalRegistros() {
        try {
            int productos = leerProductos().size();
            int ventas = leerVentas().size();
            
            // Contar operaciones de caja
            int operacionesCaja = 0;
            try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
                 PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) FROM caja_registros")) {
                
                ResultSet resultSet = statement.executeQuery();
                if (resultSet.next()) {
                    operacionesCaja = resultSet.getInt(1);
                }
            } catch (SQLException e) {
                // Si la tabla no existe, asumimos 0 operaciones
                logger.debug("Tabla caja_registros no encontrada o vacía");
                operacionesCaja = 0;
            }
            
            return new int[]{productos, ventas, operacionesCaja};
        } catch (Exception e) {
            logger.error("Error obteniendo conteo total de registros: {}", e.getMessage());
